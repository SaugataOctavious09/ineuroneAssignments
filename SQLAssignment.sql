...
show tables;
describe CityTable;
# Query all the fields where poulation > 100000
select * from CityTable 
where COUNTRYCODE="USA" and POPULATION > 100000;

# Query the name field. where population >120000
select name from CityTable 
where COUNTRYCODE="USA" and POPULATION > 120000;

# Select all columns for every row in the City Table.
select * from CityTable; 

# Query all columns where id=1661
select * from CityTable
where ID=1661;

# Query all columns where COUNTRYCODE="JPN"
select * from CityTable
where COUNTRYCODE="JPN";

# Query all names where COUNTRYCODE="JPN"
select name from CityTable
where COUNTRYCODE="JPN";

# Query list of City and State from station Table
select city, State from stationdata;

# Query a list of City names from the id that are even. Print results that are unique.
select distinct city from stationdata
where id%2 = 0;

# Difference of unique city.
select count(City) - count(distinct city) as Difference from stationdata;

#smallest City Longest City
...
# Station NAmes with aeiou

select distinct city from stationdata
where city Regexp'^[aeiouAEIOU]';

# Station NAmes end with aeiou

select distinct city from stationdata
where city Regexp'[aeiouAEIOU]$';

# Station NAmes donot start with aeiou

select distinct city from stationdata
where city not Rlike '^[aeiouAEIOU]';

# Station NAmes do not end with aeiou

select distinct city from stationdata
where city not Rlike '[aeiouAEIOU]$';


# Station NAmes do not end or start with aeiou
...
# Station NAmes do not end and start with aeiou
